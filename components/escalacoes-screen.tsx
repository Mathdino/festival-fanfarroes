...
export default function EscalacoesScreen() {
  ...
  const [novoJogador, setNovoJogador] = useState<{ nome: string; numero: string; posicaoTitular?: number }>({ nome: "", numero: "" })
  ...
  const editarJogador = (id: number, nome: string, numero: number) => {
    ...
    setJogadores(jogadores.map((j) => (j.id === id ? { ...j, nome, numero } : j)))
  ...
  const removerJogador = (id: number) => {
    ...
    const titulares = jogadoresAtualizados.filter((j) => j.posicao === "titular")
    const reservas = jogadoresAtualizados.filter((j) => j.posicao === "reserva")
    ...
    try {
      ...
      const res = await fetch("/api/times", {
        ...
        body: JSON.stringify({
          ...
          jogadores: jogadores.map(({ nome, numero, posicao, posicaoTitular }) => ({
            nome,
            numero,
            posicao,
  ...
  const abrirModalJogador = (jogador?: Jogador, posicaoTitularId?: number) => {
    if (jogador) {
      ...
      if (typeof posicaoTitularId === 'number') {
        ...
        setNovoJogador((prev) => ({ ...prev, posicaoTitular: posicaoTitularId }))
      }
    }
  ...
  const salvarJogadorModal = () => {
    ...
    if (jogadores.some((j) => j.numero === numero && j.id !== editandoJogador)) {
      alert("Número de camisa já está em uso!")
      return
    }
    if (editandoJogador) {
      ...
      setJogadores(jogadores.map((j) =>
        j.id === editandoJogador ? { ...j, nome: novoJogador.nome, numero } : j
      ))
    } else {
      // Adicionar novo jogador
      const titulares = jogadores.filter((j) => j.posicao === "titular")
      const novoId = Math.max(0, ...jogadores.map((j) => j.id)) + 1
      const posicaoTitular = novoJogador.posicaoTitular ?? (titulares.length < 6 ? titulares.length + 1 : undefined)
      const novoJogadorObj: Jogador = {
        id: novoId,
        nome: novoJogador.nome,
        numero,
      ...
      {timeSelecionado && (
        <>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setNovoJogador({ ...novoJogador, numero: e.target.value })}
                          placeholder="Número"
                          min="1"
                          max="99"
                        />
            ...
            <CardContent>
              ...
              <div className="relative bg-green-400 rounded-lg p-4 min-h-[400px] border-2 border-white">
                ...
                {posicoesTitulares.map((posicao) => {
                  const jogador = jogadores.find((j) => j.posicaoTitular === posicao.id)
                  return (
                    <div
                      key={posicao.id}
                      ...
                      <div className="bg-blue-600 text-white rounded-full w-12 h-12 flex flex-col items-center justify-center text-xs shadow-lg hover:bg-blue-700 transition-colors">
                        {jogador ? (
                          <>
                            ...
                            <span className="text-[8px] truncate w-10 text-center">{jogador.nome.split(" ")[0]}</span>
                          </>
                        ) : (
                ...
                <div className="flex gap-2 flex-wrap">
                  {jogadores
                    ...
                    .map((jogador) => (
                      ...
                      >
                        <span className="font-bold">{jogador.numero}</span>
                        <span className="truncate w-full text-center">{jogador.nome.split(" ")[0]}</span>
                      </div>
                    ))}
                  {Array.from({ length: 4 - jogadores.filter((j) => j.posicao === "reserva").length }).map(
                    (_, index) => (
                      <div
                        key={`empty-${index}`}
          ...
          {jogadores.length > 0 && (
            <Card className="bg-white/90 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="text-green-800">Lista de Jogadores</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {jogadores.map((jogador) => (
                    <div key={jogador.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      {editandoJogador === jogador.id ? (
                        <EditarJogadorForm
                          jogador={jogador}
                          onSave={(nome, numero) => editarJogador(jogador.id, nome, numero)}
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                {jogador.posicao === "titular"
                                  ? `Titular - ${posicoesTitulares.find((p) => p.id === jogador.posicaoTitular)?.nome}`
                                  : "Reserva"}
                              </p>
                            </div>
                          </div>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => setEditandoJogador(jogador.id)}>
                              <Edit className="w-3 h-3" />
                            </Button>
                            <Button size="sm" variant="destructive" onClick={() => removerJogador(jogador.id)}>
                              ×
                            </Button>
                          </div>
                        </>
          ...
          {/* Botão Salvar */}
          {jogadores.length > 0 && (
            <Button onClick={salvarEscalacao} className="w-full bg-green-600 hover:bg-green-700" disabled={carregando}>
